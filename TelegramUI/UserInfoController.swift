    ...
    if !isEditing {
        ...
        if !(peer is TelegramSecretChat) {
            ...
            if let peer = peer as? TelegramUser, peer.botInfo == nil {
                //CloudVeil start
                if MainController.shared.isSecretChatAvailable { entries.append(UserInfoEntry.startSecretChat(presentationData.theme, presentationData.strings.UserInfo_StartSecretChat))
                }
                //CloudVeil en
            }
        }
